...
class ExportDashboardBundle:
    ...
    def get_dashboard_arn(self):
        ...
        for dashboard in self.source_client.list_dashboards(AwsAccountId=self.account_id)['DashboardSummaryList']:
            if dashboard['DashboardId'] == self.dashboard_id:
                dashboard_details = dashboard
                self.dashboard_name = dashboard['Name']
                self.export_job_file_name = dashboard['Name'].replace(' ', '_').lower()
    ...
    def start_export_job(self):
        response = self.source_client.start_asset_bundle_export_job(
            ...
            ResourceArns=[self.dashboard_arn],
    ...
    def download_asset_bundle(self, response):
        ...
        bundle_filename = "{}/{}.zip".format(self.dashboard_bundle_dir, self.export_job_id)
        print('Bundle file name:{}'.format(os.path.abspath(bundle_filename)))
        urllib.request.urlretrieve(download_url, bundle_filename)
        print('Contents of asset bundles directory:{}'.format(os.listdir(self.dashboard_bundle_dir)))
        print('Asset bundle downloaded for "{}" dashboard'.format(self.dashboard_name))
